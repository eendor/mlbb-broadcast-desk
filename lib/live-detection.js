...
function prepare(state,body,now=Date.now()){
  if(!body||!['game','result'].includes(body.mode)||!Array.isArray(body.readings)||body.readings.length>60||typeof body.live!=='boolean'||typeof body.switchScene!=='boolean'||!Number.isFinite(body.sampledAt)||body.sampledAt>now+1000||body.sampledAt<now-10000)throw Error('Invalid or expired live detection');
  ...
  for(const r of body.readings){
    ...
    const valid=r.field==='winner'?['blue','red'].includes(r.value):r.field==='resultStatus'?!!Model.resultOutcome(r.value):Model.isHero(r.field)?heroes.has(r.value):OCR.parse(String(r.value),r.field)===r.value;
    if(!valid)throw Error('Invalid live detection value');
  }
  const patch={},held=[];
  if(body.switchScene)patch.scene=body.mode==='result'?'postgame':'scoreboard';
  const identities={};
  ...
  for(const r of body.readings){
    ...
    if(group==='players'){
      ...
      const team=patch[side]??={};team.bans??=[...state[side].bans];team.bans[Number(index)]=r.value;

...
function startSwiss(names) {
  ...
  for (let i = 0; i < SIZE; i += 2) {
    matches.push({ id: `R1M${i / 2 + 1}`, round: 1, pool: '0-0', blue: teams[i], red: teams[i + 1], winner: null });
...
function reportResult(swiss, matchId, winner) {
  ...
  m.winner = winner;
  if (winner === null) return { advanced: false, match: m };
...
function generateNextRound(swiss) {
  ...
  for (const key of sortedKeys) {
    ...
    for (let i = 0; i < pool.length; i += 2) {
      ...
      matches.push({ id: `R${nextRound}M${n}`, round: nextRound, pool: key, blue: a, red: b, winner: null, ...(rematch ? { rematch: true } : {}) });
...
function toSchedule(swiss) {
  return currentMatches(swiss).map(m => ({
    ...
    blue: m.blue,
    red: m.red,
    note: m.winner ? `FT • ${(m.winner === 'blue' ? m.blue : m.red).toUpperCase()} WINS` : `BO1 • ${m.pool}`,
  }));
}
...
function swissTicker(swiss) {
  ...
  let s = `SWISS R${swiss.currentRound} • ${pools} • BO1  /  USM PASIKLABAN 2026`;
...
module.exports = { SIZE, startSwiss, reportResult, currentMatches, findMatch, qualified, eliminated, renameTeams, toSchedule, swissTicker, validateSwiss, recordOf };

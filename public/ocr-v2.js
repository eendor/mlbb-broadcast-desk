...
function loadRegions(){if(profile==='auto')regions=LiveDetection.regions($('#detectCalibration').value);else try{regions=OCRModel.validateRegions(JSON.parse(localStorage.getItem(regionKey())||'null'));}catch{regions=structuredClone(OCRModel.profiles[profile]);}$('#detectControls').hidden=profile!=='auto';$('#ocrScope').closest('label').hidden=profile==='auto';$('#regions').value=JSON.stringify(regions,null,2);$('#ocrField').innerHTML=regions.map(r=>`<option>${r.field}</option>`).join('');}
...
const autoDetectButton=document.createElement('button');autoDetectButton.id='autoDetectCapture';autoDetectButton.className='primary';autoDetectButton.textContent='Start live auto-detect';$('#capture').before(autoDetectButton);
autoDetectButton.onclick=run(async()=>{autoDetectButton.disabled=true;try{stopCapture();profile='auto';$('#ocrProfile').value=profile;localStorage.setItem('ocrProfile',profile);loadRegions();$('#ocrAutoApply').checked=true;const token=generation;if(!await chooseCapture(token))return;while(ocrBusy&&token===generation)await new Promise(r=>setTimeout(r,40));if(token===generation)$('#autoOcr').click();}finally{autoDetectButton.disabled=false;}});
...
function stopLoop(){LiveDetection.stop();if(activeSyncSource==='ocr')activeSyncSource=null;ocrLoop=false;scanScheduler.cancel();generation++;$('#autoOcr').textContent='Start continuous OCR';$('#ocrProfile').disabled=false;$('#scan').disabled=ocrBusy;}
...
async function chooseCapture(token){const selected=await navigator.mediaDevices.getDisplayMedia({video:{frameRate:30},audio:false});if(token!==generation){selected.getTracks().forEach(t=>t.stop());return false;}stream=selected;video.srcObject=selected;try{await video.play();}catch(e){selected.getTracks().forEach(t=>t.stop());throw e;}if(token!==generation){selected.getTracks().forEach(t=>t.stop());return false;}selected.getVideoTracks()[0].onended=stopCapture;sourceStatus('Live');$('#ocrStatus').textContent='Live capture ready. Calibrate the boxes, then start continuous OCR.';return true;}
...
async function initWorkers(){if(workers.length)return workers;if(workerInit)return workerInit;$('#ocrStatus').textContent='Loading local OCR workers…';workerInit=(async()=>{const built=[];try{for(let i=0;i<3;i++)built.push(await Tesseract.createWorker('eng',1,{workerPath:'/vendor/tesseract/worker.min.js',corePath:location.origin+'/vendor/core',langPath:location.origin+'/vendor/lang',errorHandler:()=>{}}));workers=built;return workers;}catch(e){await Promise.all(built.map(w=>w.terminate()));throw e;}finally{workerInit=null;}})();return workerInit;}
...
if(isolateText){
  const red=r.field.startsWith('red.'),level=r.field.endsWith('.level');
  for(let i=0;i<pixelsFull.data.length;i+=4){
    ...
    const team=!level&&(red?rr>160&&g>95&&b>95&&rr>g*1.1&&rr>b*1.05:g>145&&b>145&&g>rr*1.08&&b>rr*1.08);
    const n=neutral||team?0:255;pixelsFull.data[i]=pixelsFull.data[i+1]=pixelsFull.data[i+2]=n;
  }
...
$('#autoOcr').onclick=run(()=>{if(ocrLoop){stopLoop();$('#ocrStatus').textContent='Continuous OCR stopped.';api('/api/ocr/stop',{}).catch(()=>{});return;}if(!source())throw Error('Choose a capture source first');if(ocrBusy)throw Error('Wait for the current scan to finish');if(poll){clearInterval(poll);poll=null;$('#pollMatch').textContent='Start auto-sync';toast('Match API auto-sync stopped while OCR runs');}activeSyncSource='ocr';$('#ocrAutoApply').checked=true;ocrLoop=true;generation++;$('#autoOcr').textContent='Stop continuous OCR';$('#ocrProfile').disabled=true;nextScan();});

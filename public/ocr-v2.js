...
let stream=null,stillImage=null,clipUrl=null,workers=[],workerInit=null,ocrBusy=false,ocrLoop=false,loopTimer=null,generation=0,dragStart=null,ocrReadings=[],regions=[];
let profile=localStorage.getItem('ocrProfile')||'scoreboard';if(!OCRModel.profiles[profile])profile='scoreboard';
...
const controls=document.createElement('div');controls.innerHTML=`<div class="fields"><label>OCR layout<select id="ocrProfile"><option value="scoreboard">Game scoreboard</option><option value="players">Player rails (reference crop)</option><option value="draft">Draft phase + seconds (reference crop)</option></select></label><label>Scan scope<select id="ocrScope"><option value="all">All calibrated fields</option><option value="selected">Selected field only</option></select></label><label>Confirm across frames<select id="ocrStability"><option value="2">2 readings</option><option value="3">3 readings</option><option value="1">1 reading (fastest)</option></select></label></div><p class="ocr-profile-note">Capture the clean game feed before your overlay is added. Player and draft presets need the matching crop; recalibrate boxes for a full game window.</p><div class="ocr-health"><span id="ocrSpeed">Recognition: idle</span><span id="ocrDelivery">Delivery: idle</span><span id="ocrSourceSize">No capture</span></div><label class="check"><input id="ocrSmoothClock" type="checkbox" checked> Keep the game clock ticking between live readings</label><div class="buttonrow"><label class="filebutton">Open test video<input id="ocrClip" type="file" accept="video/*"></label><button id="ocrResetTracking">New game / clear OCR history</button></div>`;
$('#captureCanvas').before(controls);$('#ocrProfile').value=profile;$('#autoOcr').textContent='Start continuous OCR';$('#confidence').value=localStorage.getItem('ocrConfidence')||'80';
function loadRegions(){try{regions=OCRModel.validateRegions(JSON.parse(localStorage.getItem('ocrRegions:'+profile)||'null'));}catch{regions=structuredClone(OCRModel.profiles[profile]);}$('#regions').value=JSON.stringify(regions,null,2);$('#ocrField').innerHTML=regions.map(r=>`<option>${r.field}</option>`).join('');}
...
loadRegions();
function invalidate(){generation++;clearTracking();}
...
$('#capture').onclick=run(async()=>{stopCapture();stream=await navigator.mediaDevices.getDisplayMedia({video:{frameRate:30},audio:false});video.srcObject=stream;await video.play();stream.getVideoTracks()[0].onended=stopCapture;sourceStatus('Live');$('#ocrStatus').textContent='Live capture ready. Calibrate the boxes, then start continuous OCR.';});
...
video.addEventListener('seeking',invalidate);video.addEventListener('ended',()=>{stopLoop();api('/api/ocr/stop',{}).catch(()=>{});});
...
captureCanvas.onpointerup=e=>{if(!dragStart)return;const p=point(e),r=regions.find(r=>r.field===$('#ocrField').value),w=Math.abs(p.x-dragStart.x),h=Math.abs(p.y-dragStart.y);if(r&&w>3&&h>3){const x=Math.floor(Math.min(p.x,dragStart.x)),y=Math.floor(Math.min(p.y,dragStart.y));Object.assign(r,{x,y,w:Math.min(1920-x,Math.floor(w)),h:Math.min(1080-y,Math.floor(h))});$('#regions').value=JSON.stringify(regions,null,2);localStorage.setItem('ocrRegions:'+profile,JSON.stringify(regions));invalidate();}dragStart=null;drawCapture();};
$('#saveRegions').onclick=run(()=>{regions=OCRModel.validateRegions(JSON.parse($('#regions').value));localStorage.setItem('ocrRegions:'+profile,JSON.stringify(regions));$('#ocrField').innerHTML=regions.map(r=>`<option>${r.field}</option>`).join('');invalidate();toast('Calibration saved');});
async function initWorkers(){if(workers.length)return workers;if(workerInit)return workerInit;$('#ocrStatus').textContent='Loading two local OCR workers…';workerInit=(async()=>{const built=[];try{for(let i=0;i<2;i++)built.push(await Tesseract.createWorker('eng',1,{workerPath:'/vendor/tesseract/worker.min.js',corePath:location.origin+'/vendor/core',langPath:location.origin+'/vendor/lang'}));workers=built;return workers;}catch(e){await Promise.all(built.map(w=>w.terminate()));throw e;}finally{workerInit=null;}})();return workerInit;}
function crop(r){const s=source();if(!s)throw Error('Choose a game window, screenshot, or test video');const sw=s.videoWidth||s.width,sh=s.videoHeight||s.height,sx=sw/1920,sy=sh/1080,c=document.createElement('canvas'),scale=Math.max(1,Math.min(4,80/(r.h*sy)));c.width=Math.max(1,Math.round(r.w*sx*scale));c.height=Math.max(1,Math.round(r.h*sy*scale));const cx=c.getContext('2d');cx.filter='grayscale(1) contrast(1.3)';cx.drawImage(s,r.x*sx,r.y*sy,r.w*sx,r.h*sy,0,0,c.width,c.height);const tiny=document.createElement('canvas');tiny.width=32;tiny.height=12;const tx=tiny.getContext('2d',{willReadFrequently:true});tx.drawImage(c,0,0,32,12);const pixels=tx.getImageData(0,0,32,12).data,signature=new Uint8Array(384);for(let i=0;i<signature.length;i++)signature[i]=pixels[i*4];// Normalize light digits on dark team colors to dark text on white, with OCR margin.
...
samples.sort((a,b)=>a-b);if(samples[Math.floor(samples.length/2)]<145){for(let i=0;i<pixelsFull.data.length;i+=4){const n=255-pixelsFull.data[i];pixelsFull.data[i]=pixelsFull.data[i+1]=pixelsFull.data[i+2]=n;}cx.putImageData(pixelsFull,0,0);}
...
async function applyOcr(){if(!ocrReadings.length)throw Error('No accepted readings');await publish(ocrReadings,Date.now(),false,generation);}
async function scan(continuous=false){
 ...
 try{
  ...
  async function readRegion(worker,r,attempt=0){
   const item=crop(r),old=cache.get(r.field),changed=OCRRuntime.changed(old?.signature,item.signature),clockField=['gameTime','draftTimer.remaining'].includes(r.field);
   ...
   if(continuous&&accepted&&previous!==undefined&&typeof value==='number'&&!clockField&&value<previous){accepted=false;reason='decrease held';}
...
function priority(field){return field==='gameTime'||field==='draftTimer.remaining'?0:field.endsWith('.kills')?1:field.endsWith('.name')?3:2;}
async function nextScan(){if(!ocrLoop)return;try{await scan(true);}catch(e){stopLoop();toast(e.message,true);$('#ocrStatus').textContent='OCR stopped: '+e.message;return;}if(ocrLoop)loopTimer=setTimeout(nextScan,80);}
$('#scan').onclick=run(()=>scan(false));$('#applyOcr').onclick=run(async()=>{await applyOcr();toast('Accepted readings applied');});
$('#autoOcr').onclick=run(()=>{if(ocrLoop){stopLoop();$('#ocrStatus').textContent='Continuous OCR stopped.';api('/api/ocr/stop',{}).catch(()=>{});return;}if(!source())throw Error('Choose a capture source first');if(poll){clearInterval(poll);poll=null;$('#pollMatch').textContent='Start auto-sync';toast('Match API auto-sync stopped while OCR runs');}activeSyncSource='ocr';ocrLoop=true;generation++;$('#autoOcr').textContent='Stop continuous OCR';$('#ocrProfile').disabled=true;nextScan();});
window.addEventListener('beforeunload',()=>{generation++;stream?.getTracks().forEach(t=>t.stop());workers.forEach(w=>w.terminate());});

...
// Cut out the background of an uploaded photo (MVP player shots) with rembg.
// Falls back to the original upload if the Python tool or model is unavailable.
app.post('/api/photo/cutout',(req,res)=>{
  const src=String(req.body.url||'');
  if(!/^\/assets\/uploads\/[a-f0-9]{64}\.(png|jpe?g|webp)$/.test(src))throw Error('Upload a PNG/JPG/WebP photo first');
  const input=path.join(__dirname,'public',src);
  if(!fs.existsSync(input))throw Error('Uploaded photo not found');
  const outName=require('node:crypto').createHash('sha256').update(src+':cutout').digest('hex')+'.png';
  const outRel='/assets/uploads/'+outName,output=path.join(__dirname,'public/assets/uploads',outName);
  if(fs.existsSync(output))return res.json({url:outRel,cached:true});
  const py=process.platform==='win32'?path.join(dataDir,'ocr-venv/Scripts/python.exe'):path.join(dataDir,'ocr-venv/bin/python');
  const python=fs.existsSync(py)?py:(process.platform==='win32'?'python':'python3');
  const {execFile}=require('node:child_process');
  execFile(python,[path.join(__dirname,'scripts/remove-bg.py'),input,output],{timeout:120000},(err)=>{
    if(err||!fs.existsSync(output)){console.error('Background removal failed:',err?.message||'no output');return res.json({url:src,cutout:false,error:'Background removal unavailable; using original photo.'});}
    res.json({url:outRel,cutout:true});
  });
});
